...
const S = {
  ...
  get settings() {
    const DEFAULTS = {
      ...
      maxPerCategory:     2,
      watchCompanies: [
        { name: '本田技研工業',           code: '7267', prtimes_id: '51843'  },
        { name: 'レゾナック',             code: '4004', prtimes_id: '102176' },
        { name: '神戸製鋼所',             code: '5406', prtimes_id: '125552' },
        { name: 'KDDI',                   code: '9433', prtimes_id: '13141'  },
        { name: '日本酸素ホールディングス', code: '4091', prtimes_id: '118246' },
        { name: '渡辺パイプ',             code: null,   prtimes_id: '46508'  },
        { name: '五洋建設',               code: '1893', prtimes_id: null     },
        { name: '日清紡マイクロデバイス', code: null,   prtimes_id: '71712'  },
        { name: 'オムロン',               code: '6645', prtimes_id: '120244' },
        { name: 'トヨタ車体',             code: '7975', prtimes_id: '173179' },
...
const RSS_SOURCES = [
  ...
  // ── 主要系強化 ──
  { url: 'https://www.yomiuri.co.jp/feed/', source: '読売新聞', category: '主要' },
  { url: 'https://www.jiji.com/rss/ranking.rdf', source: '時事通信', category: '主要' },
  { url: 'https://www.jiji.com/rss/pol.rdf', source: '時事通信', category: '政治' },
  { url: 'https://www.jiji.com/rss/eco.rdf', source: '時事通信', category: '経済' },
  { url: 'https://www.jiji.com/rss/soc.rdf', source: '時事通信', category: '社会' },
  { url: 'https://www.jiji.com/rss/int.rdf', source: '時事通信', category: '国際' },
  { url: 'https://www.jiji.com/rss/spo.rdf', source: '時事通信', category: 'スポーツ' },
  { url: 'https://www.nikkansports.com/rss/news.rdf', source: '日刊スポーツ', category: 'スポーツ' },
  { url: 'https://feeds.reuters.com/reuters/JPBusinessNews', source: 'Reuters Japan 経済', category: '経済' },
  ...
  for (const topicStr of newTopics) {
    ...
    if (idx >= 0) {
      ...
      if (arr.length >= TOPIC_LIMIT) {
        ...
        const evict = arr.findIndex(t => t.weight === minW);
        if (evict >= 0) arr.splice(evict, 1);
      }
...
async function generateDuoScript(items, cfg) {
  const duoTimePerItem = cfg.timePerItem || 1;
  const duoLengthMin   = Math.round(items.length * duoTimePerItem);
  const newsText  = items.map((n, i) => `${i+1}. 【${n.category}】${n.title}\n${n.summary || ''}`).join('\n\n');
...
- 提供した全ニュースを必ず含めること（全${items.length}件）、1件あたり約${duoTimePerItem}分
...
function readDeepDiveText(btn) {
  ...
  // 再クリックで停止
  if (resultDiv._playing) {
    resultDiv._playing = false;
    window.speechSynthesis.cancel();
  ...
  (function speakNext() {
    if (!resultDiv._playing || idx >= chunks.length) {
      resultDiv._playing = false;
      btn.textContent = '▶ 読み上げ';
      return;
    }
...
- 数字は日本語の読みに合わせて表記（例: 2025年→二〇二五年、1兆円→一兆円）
- 英語・アルファベット略語はカタカナのみで書き、アルファベット表記は一切使わない（例: AI→エーアイ、GDP→ジーディーピー）
- 文末は必ず「。」で終わらせる`;
...
function updateTotalTime() {
  const count = parseInt($('setting-max').value, 10) || 15;
  const time  = parseFloat($('setting-time-per-item')?.value) || 1;
  const total = Math.round(count * time);
  const el    = $('setting-total-time');
  if (el) el.textContent = `→ 合計約${total}分`;
}
...
function populateSettings() {
  ...
  $('setting-time-per-item').value = String(cfg.timePerItem ?? 1);
...
function saveSettings() {
  ...
  S.saveSettings({
    ...
    timePerItem:      parseFloat($('setting-time-per-item').value) || 1,
...
async function callClaude(system, userMsg) {
  ...
  if (!data.content?.length) throw new Error('APIから空の応答が返りました');
